...
def nomeFornecedor(filename, full_name=False):
  ...
  vetorResul = re.findall(r'AGV LOGISTICA(?: SA)?|RIO LOPES TRANSPORTES(?: LTDA)?|GKO INFORMATICA LTDA|MOVEIDEIAS CONSULTORIA E INTEGRACAO DE NEGOCIOS LTDA - EPP|RODOLOG TRANSPORTES MULTIMODAIS(?: LTDA)?|RUNTEC(?: INFOMATICA)?(?: LTDA)?|SHIFT(?: GESTAO)?(?: DE)?(?: SERVICOS)?(?: LTDA)?|DENISE DOS ANJOS PINTO LUCENA|DIREMADI(?: MARKETING)?(?: E)?(?: SERVICOS)?(?: LTDA)?|LINE EXPRESS(?: TRANSPORTES)?(?: E)?(?: DISTRIBUICAO)?(?: LTDA)?|LINEX|ANDREANI(?: LOGISTICA)?|FL ?BRASIL(?: HOLDING)?|SENIOR(?: SISTEMAS)?(?: S/?A)?|MULTI RIO(?: OPERACOES)?(?: PORTUARIAS)?|MULTITERMINAIS|ICTSI(?: RIO)(?: BRASIL) TERMINAL 1 SA|DHL(?: GLOBAL)?(?: FORWARDING)?(?: \( BRAZIL\))?(?: LOGISTICS LTDA|KUEHNE(?:.)?NAGEL)?', texto, flags=re.I)
  if len(vetorResul) > 0:
